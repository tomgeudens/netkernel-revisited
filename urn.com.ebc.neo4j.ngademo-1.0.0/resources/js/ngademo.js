...
document.addEventListener("DOMContentLoaded", function() {
	...
	cy = cytoscape({
		...
		style: fetch("/ngademo/json/ngademo_style.json", {mode: "no-cors"})
		.then(function(res) {
			return res.json()
		})
	});
	...
	.then(function(names) {
		names.forEach(function(name) {
			$('#artistselect').append('<option value="' + name + '">' +  name + '</option>');
		});
	});
	...
	cy.ready(function(event) {
		...
		// catch the button
		$("#artistform").submit(function(ev) {
			ev.preventDefault();
			visualizeArtist($("#artistselect").val());
		});
...
//getting the Artist names for the select field
function getArtistNames() {
	var session = driver.session();
	
	return session.run("CALL apoc.index.search('nga', 'Entity.name:pollock') YIELD node RETURN node.name AS name;")
	/*
...
//adding the Artist node to the visualization
function visualizeArtist(artistname) {
	...
	return session.run('MATCH (e:Entity) WHERE e.name = "' + artistname + '" RETURN e;')
	.then(result => {
		session.close();
		
		if (result.records.length == 0) {
			console.log("Artist " + artistname + " was not found in the database.");
			return {};
		}

		var artistnode = result.records[0].get("e");
		
		var output = {
				id: "node_" + artistnode.identity.low,
				neo4j_id: artistnode.identity.low,
				neo4j_label: artistnode.labels.join()
		}

		Object.keys(artistnode.properties).forEach(key => {
			output[key] = artistnode.properties[key];
		});
