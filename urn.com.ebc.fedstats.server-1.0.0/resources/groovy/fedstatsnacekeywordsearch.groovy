...
/**
 * Fedstats Nace Keyword Search Accessor
 */
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNaceKWSAccessor: start of id - " + vId);
//
...
kwsrequest.addArgument("database","fedstats:database-nace");
kwsrequest.addArgument("expiry","fedstats:expiry-nace");
kwsrequest.addArgument("credentials","fedstats:credentials-nace");
...
if (vHTTPResponseCode >= 400) {
	...
	if (aAccept.startsWith("text/html")) {
		...
		xsltcrequest.addArgument("operator", "res:/resources/xsl/fedstatsnacekeywordsearch.xsl");
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNaceKWSAccessor: finish of id - " + vId + ", duration was " + vElapsedSeconds + " seconds");
//