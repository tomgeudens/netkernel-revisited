...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNISSPARQLAccessor: start of id - " + vId);
//
...
if (aQuery == null || aQuery == "") {
	try {
		aQuery = (String)aContext.source("fedstats:query-nis", String.class);
	}
	catch (Exception e) {
		// sensible default
...
if (vHTTPResponseCode >= 400) {
	...
	if (aAccept.startsWith("text/html")) {
		if (vConstructFound || vDescribeFound || vAskFound) {
			...
			xsltcrequest.addArgument("operator", "res:/resources/xsl/fedstatsnissparql.xsl");
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNISSPARQLAccessor: finish of id - " + vId + ", duration was " + vElapsedSeconds + " seconds");
//