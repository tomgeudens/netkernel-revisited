...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNaceSPARQLAccessor: start of id - " + vId);
//
...
if (aQuery == null || aQuery == "") {
	try {
		aQuery = (String)aContext.source("fedstats:query-nace", String.class);
	}
	catch (Exception e) {
		// sensible default
...
if (vHTTPResponseCode >= 400) {
	...
	if (aAccept.startsWith("text/html")) {
		if (vConstructFound || vDescribeFound || vAskFound) {
			...
			xsltcrequest.addArgument("operator", "res:/resources/xsl/fedstatsnacesparql.xsl");
			xsltcrequest.addArgument("replace","fedstats:baseurl");
			xsltcrequest.addArgument("with","fedstats:activeurl");
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNaceSPARQLAccessor: finish of id - " + vId + ", duration was " + vElapsedSeconds + " seconds");
//