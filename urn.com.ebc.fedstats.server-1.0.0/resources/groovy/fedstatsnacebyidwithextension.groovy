...
/**
 * Fedstats Nace by ID with Extension Accessor
 */
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNacebyIDwithExtensionAccessor: start of id - " + vId);
//
...
try {
	...
	aContext.logRaw(INKFLocale.LEVEL_WARNING, "FedstatsNacebyIDwithExtensionAccessor: invalid - extension - argument");
	// sensible default
	aExtension = "html"
}
//
...
// processing
INKFRequest nacebyidrequest = aContext.createRequest("active:fedstatsnacebyid");
nacebyidrequest.addArgument("owner","arg:owner");
nacebyidrequest.addArgument("id","arg:id");

INKFResponseReadOnly nacebyidresponse = aContext.issueRequestForResponse(nacebyidrequest);
int vHTTPResponseCode = nacebyidresponse.getHeader("httpresponsecode");
Object vNacebyIDResult = nacebyidresponse.getRepresentation();
//
...
if (vHTTPResponseCode >= 400) {
	vResponse = aContext.createResponseFrom(vNacebyIDResult);
	...
	case "rdf":
		...
		rdfxml2rdfxmlrequest.addArgumentByValue("operand", vNacebyIDResult);
	...
	case "ttl":
		...
		rdfxml2turtlerequest.addArgumentByValue("operand", vNacebyIDResult);
	...
	case "nt":
		...
		rdfxml2ntriplerequest.addArgumentByValue("operand", vNacebyIDResult);
	...
	case "jsonld":
		...
		rdfxml2jsonldrequest.addArgumentByValue("operand", vNacebyIDResult);
	...
	case "html":
		...
		rdfxml2htmlrequest.addArgumentByValue("operand", vNacebyIDResult);
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "FedstatsNacebyIDwithExtensionAccessor: finish of id - " + vId + ", duration was " + vElapsedSeconds + " seconds");
//