...
<xsl:stylesheet
	...
	exclude-result-prefixes="sparql nk">
	...
	<xsl:template match="/sparql:sparql">
		<xsl:for-each select="sparql:results/sparql:result">
			...
			<xsl:value-of select="sparql:binding[@name='id']/sparql:uri"/><xsl:text>",</xsl:text>
			
			<xsl:text>"score":"</xsl:text>
			<xsl:value-of select="sparql:binding[@name='score']/sparql:literal"/><xsl:text>",</xsl:text>
			
			<xsl:text>"name":"</xsl:text>
			<xsl:value-of select="sparql:binding[@name='label']/sparql:literal"/><xsl:text>",</xsl:text>
			
			<xsl:text>"type": ["</xsl:text>
			<xsl:value-of select="sparql:binding[@name='type']/sparql:uri"/><xsl:text>"],</xsl:text>
