...
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoDEFAccessor: start of id - " + vId);
//
...
// processing
INKFRequest incacherequest = aContext.createRequest("pds:/def");
...
if (vInCache) {
	vSPARQLResult = aContext.source("pds:/def");
	vHTTPResponseCode = 200;
}
else {
	INKFRequest sparqlrequest = aContext.createRequest("active:sparql");
	sparqlrequest.addArgument("database","milieuinfo:database-def");
	sparqlrequest.addArgument("expiry", "milieuinfo:expiry-def");
	sparqlrequest.addArgument("credentials","milieuinfo:credentials-def");
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoDEFAccessor: finish of id - " + vId + ", duration was " + vElapsedSeconds + " seconds");
//