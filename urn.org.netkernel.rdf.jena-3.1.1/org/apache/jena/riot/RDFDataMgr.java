...
import org.netkernel.rdf.jena.endpoint.QueryModelAccessor;
...
public class RDFDataMgr
{
    ...
    /** 
     * 1060 Research - Rewritten to use ROC Context to SOURCE resource references as parsed Model (would be nice if we could pass URIResolver like with XML)
     * 	PJR 20160929
     *
    ...
    public static void read(Model model, String uri, String base, Lang hintLang) { 
    	...
    	*/
    	//Using static ThreadLocal is an evil hack because Jena is not abstracted to load resources!!!
    	INKFRequestContext context=QueryModelAccessor.getMyThreadLocalContext();
    	try
    	{	IRepJenaModel m=context.source(uri, IRepJenaModel.class);
    		model.add(m.getModel());
    	}
    	catch(Exception e)
    	{	//Nothing we can do down here in the depths
    		throw new RiotNotFoundException(e);
    	}
    }
...
//    public static DatasetGraph loadDatasetGraph(String uri, String base, Lang lang, Context context) { return null ; } 
    ...
    /** 
     * 1060 Research - Rewritten to use ROC Context to SOURCE resource references (would be nice if we could pass URIResolver like with XML)
     * PJR 20160929
     * 
     * Open a stream to the destination (URI or filename)
     * Performs content negotiation, including looking at file extension. 
    ...
    public static TypedInputStream open(String filenameOrURI, StreamManager streamManager) {
    	...
    	INKFRequestContext context=QueryModelAccessor.getMyThreadLocalContext();
    	try
    	{	INKFResponseReadOnly<IReadableBinaryStreamRepresentation> resp=context.sourceForResponse(filenameOrURI, IReadableBinaryStreamRepresentation.class);
    		String mime=resp.getMimeType();
    		if(mime==null || mime.equals("content/unknown"))
    		{	Lang lang=RDFLanguages.filenameToLang(filenameOrURI);
    			if(lang!=null)
    			{	mime=lang.getContentType().getContentType();
    			}
    		}
    		TypedInputStream in=new TypedInputStream(resp.getRepresentation().getInputStream(), mime);
    		return in;
    	}
    	catch(Exception e)
    	{	//Nothing we can do down here in the depths
