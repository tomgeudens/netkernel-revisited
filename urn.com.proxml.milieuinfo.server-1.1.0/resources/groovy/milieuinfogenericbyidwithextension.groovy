...
try {
	...
	aContext.logRaw(INKFLocale.LEVEL_SEVERE, "MilieuinfoGenericbyIDwithExtensionAccessor: ("
		+ vId
		+ ") - argument owner : invalid");
	throw new Exception("MilieuinfoGenericbyIDwithExtensionAccessor: no valid - owner - argument");
}
String vDomain = aOwner.substring(0, aOwner.lastIndexOf("/"));
INKFRequest getdatabaserequest = aContext.createRequest("active:getdatabase");
getdatabaserequest.addArgumentByValue("domain", vDomain);
...
try {
	...
	aContext.logRaw(INKFLocale.LEVEL_SEVERE, "MilieuinfoGenericbyIDwithExtensionAccessor: ("
		+ vId
		+ ") - argument id : invalid");
	throw new Exception("MilieuinfoGenericbyIDwithExtensionAccessor: no valid - id - argument");
}
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoGenericbyIDwithExtensionAccessor: ("
	+ vId
	+ ") - argument id : "
...
String vFullURI = vBaseURL + "/" + aOwner + "/" + aID;
//
...
IHDSDocument vCatalog = aContext.source("res:/resources/xml/id-sparql-catalog.xml", IHDSDocument.class);
...
for (IHDSReader vQueryReader: vCatalogReader.getNodes("/sparql-catalog/sparql")) {
	...
	for (IHDSReader vConditionReader: vQueryReader.getNodes("condition")) {
		...
		if (vParameter == "domain" && vHasNotValue != null && vHasNotValue == vDomain) {
			vProcess = false;
			break;
		}
		...
		if (vParameter == "domain" && vHasValue != null && vHasValue != vDomain) {
			vProcess = false;
			break;
		}
	...
	if (vProcess) {
		...
		freemarkerrequest.addArgumentByValue("domain", vDomain);
...
case "html":
	...
	rdfxml2htmlsaxonrequest.addArgumentByValue("domain", vDomain);
