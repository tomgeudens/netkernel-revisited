...
/**
 * Milieuinfo GPBV by ID with Extension Accessor
 */
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoGPBVbyIDwithExtensionAccessor: start of id - " + vId);
//
...
try {
	...
	aContext.logRaw(INKFLocale.LEVEL_WARNING, "MilieuinfoGPBVbyIDwithExtensionAccessor: invalid - extension - argument");
	// sensible default
	aExtension = "html"
}
//
...
// processing
INKFRequest gpbvbyidrequest = aContext.createRequest("active:milieuinfogpbvbyid");
gpbvbyidrequest.addArgument("owner","arg:owner");
gpbvbyidrequest.addArgument("id","arg:id");

INKFResponseReadOnly gpbvbyidresponse = aContext.issueRequestForResponse(gpbvbyidrequest);
int vHTTPResponseCode = gpbvbyidresponse.getHeader("httpresponsecode");
Object vGPBVbyIDResult = gpbvbyidresponse.getRepresentation();
//
...
if (vHTTPResponseCode >= 400) {
	vResponse = aContext.createResponseFrom(vGPBVbyIDResult);
	...
	case "rdf":
		...
		rdfxml2rdfxmlrequest.addArgumentByValue("operand", vGPBVbyIDResult);
	...
	case "ttl":
		...
		rdfxml2turtlerequest.addArgumentByValue("operand", vGPBVbyIDResult);
	...
	case "nt":
		...
		rdfxml2ntriplerequest.addArgumentByValue("operand", vGPBVbyIDResult);
	...
	case "jsonld":
		...
		rdfxml2jsonldrequest.addArgumentByValue("operand", vGPBVbyIDResult);
	...
	case "html":
		...
		rdfxml2htmlsaxonrequest.addArgumentByValue("operand", vGPBVbyIDResult);
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoGPBVbyIDwithExtensionAccessor: finish of id - " + vId + ", duration was " + vElapsedSeconds + " seconds");
//