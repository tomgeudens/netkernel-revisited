...
/**
 * Milieuinfo Generic KeyWordSearch Accessor
 */
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoGenericKeyWordSearchAccessor: ("
		+ vId
		+ ") - start");
//
...
try {
	...
	aContext.logRaw(INKFLocale.LEVEL_SEVERE, "MilieuinfoGenericKeyWordSearchAccessor: ("
		+ vId
...
kwsrequest.addArgument("database","milieuinfo:database-" + vDatabase);
kwsrequest.addArgument("expiry","milieuinfo:expiry-" + vDatabase);
kwsrequest.addArgument("credentials","milieuinfo:credentials-" + vDatabase);
...
if (vHTTPResponseCode >= 400) {
	...
	if (aAccept.startsWith("text/html")) {
		...
		xslt2request.addArgument("operator", "res:/resources/xsl/keywordsearchgeneric.xsl");
