...
/**
 * Milieuinfo GPBV VOID with Extension Accessor
 */
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoGPBVVOIDwithExtensionAccessor: start of id - " + vId);
//
...
try {
	...
	aContext.logRaw(INKFLocale.LEVEL_WARNING, "MilieuinfoGPBVVOIDwithExtensionAccessor: invalid - extension - argument");
	// sensible default
	aExtension = "html"
}
//
...
// processing
INKFRequest gpbvvoidrequest = aContext.createRequest("active:milieuinfogpbvvoid");

INKFResponseReadOnly gpbvvoidresponse = aContext.issueRequestForResponse(gpbvvoidrequest);
int vHTTPResponseCode = gpbvvoidresponse.getHeader("httpresponsecode");
Object vGPBVVOIDResult = gpbvvoidresponse.getRepresentation();
//
...
if (vHTTPResponseCode >= 400) {
	vResponse = aContext.createResponseFrom(vGPBVVOIDResult);
	...
	case "rdf":
		...
		rdfxml2rdfxmlrequest.addArgumentByValue("operand", vGPBVVOIDResult);
	...
	case "ttl":
		...
		rdfxml2turtlerequest.addArgumentByValue("operand", vGPBVVOIDResult);
	...
	case "nt":
		...
		rdfxml2ntriplerequest.addArgumentByValue("operand", vGPBVVOIDResult);
	...
	case "jsonld":
		...
		rdfxml2jsonldrequest.addArgumentByValue("operand", vGPBVVOIDResult);
...
aContext.logRaw(INKFLocale.LEVEL_INFO, "MilieuinfoGPBVVOIDwithExtensionAccessor: finish of id - " + vId + ", duration was " + vElapsedSeconds + " seconds");
//